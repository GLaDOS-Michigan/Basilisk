...
module DistributedSystem {
  ...
  datatype Constants = Constants(f: nat, hosts: seq<Host.Constants>, logCap: int) {
    ghost predicate WF()
    {
      && 0 < f
      && UniqueIds()
    ...
    {
      forall i: HostId, j: HostId | ValidHostIdx(i) && ValidHostIdx(j) && hosts[i].id == hosts[j].id :: i == j
    }

    ghost predicate ValidHostIdx(id: int) {
      0 <= id < |hosts|
    }

    ghost predicate ValidSlot(slot: int) {
      0 <= slot < logCap
    }
  }
  ...
  datatype Hosts = Hosts(hosts: seq<Host.Variables>) {
    ...
    {
      c.WF() &&
      Host.GroupWF(c.hosts, hosts, c.f, c.logCap)
    }

    ghost predicate LeaderCanPropose(c: Constants, ldr: HostId)
      requires WF(c)
      requires c.ValidHostIdx(ldr)
      decreases this, c, ldr
    {
      hosts[ldr].LdrCanPropose(c.hosts[ldr])
    }
  }
  // end datatype Hosts
  ...
  ghost predicate InitHosts(c: Constants, h: Hosts)
    requires h.WF(c)
  {
    Host.GroupInit(c.hosts, h.hosts, c.f, c.logCap)
  }
    ...
    requires h.WF(c) && h'.WF(c)
  {
    ...
    && NextHostStep(c, h, h', step.actor, step.msgOps)
  }
