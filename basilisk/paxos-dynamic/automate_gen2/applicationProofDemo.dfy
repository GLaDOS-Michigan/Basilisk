...
lemma GetIntersectingAcceptor(c: Constants, v: Variables, accQ: set<Message>, accVB: ValBal,  promQ: set<Message>, promBal: Ballot)
...
returns (promS: set<Message>)
  ...
  ensures LeaderPromiseSetProperties(c, v, i, ldrBal, promS)
{
  promS := {};

  var ldr := v.History(i).hosts[ldrBal.id];
  var hbal := ldr.ls.highestHeardBallot;
  ...
  if hbal.Some? {
    ...
    while |accs| > 0 
      ...
      invariant hbal.None? ==> PromiseSetEmptyVB(c, v, promS, ldr.ls.currBal)
      invariant MessageSetDistinctAccs(promS)
      invariant forall p: Message | p in promS :: p.acc in ldr.LdrReceivedPromises()
      invariant WinningPromiseMessageInQuorum(c, v, promS, ldrBal, VB(ldr.LdrValue(), hbal.value), hm)
      decreases accs
    {
      var acc :| acc in accs;
      var p := PromiseMessageExistence(c, v, i, ldrBal, acc);
      promS := promS + {p};
      accs := accs - {acc};
      assert MessageSetDistinctAccs(promS);  // trigger
    }
...
lemma PromiseMessageExistence(c: Constants, v: Variables, i: int, ldrBal: Ballot, acc: HostId) 
  ...
  requires HostLsMonotonic(c, v)
  requires HostReceiveValidity(c, v)
  requires ReceivePromise1ReceivePromise2ReceivePreempt1ReceivePreempt2WitnessCondition(c, v, i, ldrBal.id, acc)
  requires v.History(i).hosts[ldrBal.id].ls.currBal == ldrBal
  ensures   && promiseMsg.Promise?
            ...
            && (promiseMsg.vbOpt.Some? ==> 
                && v.History(i).hosts[ldrBal.id].ls.highestHeardBallot.Some?
                && BalLteq(promiseMsg.vbOpt.value.b, v.History(i).hosts[ldrBal.id].ls.highestHeardBallot.value)  
            )
{
  reveal_ValidHistory();
...
lemma LearnerValidReceivedAccepts(c: Constants, v: Variables, i: nat, lnr: HostId) 
  ...
  requires ValidMessages(c, v)
  requires HostReceiveValidity(c, v)
  requires HostReceivedAcceptsMonotonic(c, v)
...
lemma LearnedImpliesQuorumOfAccepts(c: Constants, v: Variables, lnr: HostId, val: Value) 
  ...
  ensures exists b: Ballot :: ChosenAtLearner(c, v.Last(), VB(val, b), lnr)
{
    reveal_ValidHistory();
    reveal_ChosenAtLearner();
...
ghost predicate WinningPromiseMessageInQuorum(c: Constants, v: Variables, pset: set<Message>, qbal: Ballot, hsvb: ValBal, m: Message)
  requires IsPromiseSet(c, v, pset, qbal)
{
    && m in pset 
    && m.vbOpt == Some(hsvb)
    && (forall other | other in pset  && other.vbOpt.Some?
        :: BalLteq(other.vbOpt.value.b, hsvb.b))
}

ghost predicate PromiseSetHighestVB(c: Constants, v: Variables, pset: set<Message>, qbal: Ballot, hsvb: ValBal)
  requires IsPromiseSet(c, v, pset, qbal)
{
...
ghost predicate PromiseSetEmptyVB(c: Constants, v: Variables, pset: set<Message>, qbal: Ballot)
  requires IsPromiseSet(c, v, pset, qbal)
{
  forall m | m in pset :: m.vbOpt == None
}

ghost predicate LeaderPromiseSetProperties(c: Constants, v: Variables, i: nat, ldrBal: Ballot, promS: set<Message>) 
  requires v.WF(c)
  requires v.ValidHistoryIdx(i)
  requires c.ValidHostIdx(ldrBal.id)
  requires v.History(i).hosts[ldrBal.id].ls.currBal == ldrBal
{
    ...
    && (hbal.Some? ==> PromiseSetHighestVB(c, v, promS, ldr.ls.currBal, VB(ldr.LdrValue(), hbal.value)))
    && (hbal.None? ==> PromiseSetEmptyVB(c, v, promS, ldr.ls.currBal))
