  ...
  decreases vb2.b, 0
{
  ...
  if hb == vb1.b {
    ...
    var propMsg1 := ChosenImpliesProposed(c, v, |v.history|-1, vb1);
    assert propMsgOps.recv.value.val == propMsg1.val;     // trigger
  } else {
    // Do induction
    var nq, nb := GetPromiseQuorumForProposeMessage(c, v, vb1, propMsgOps.recv.value);
  ...
  ensures chosenVB.b <= promMsg.vbOpt.value.b
{
  ...
  */
  ...
  ChosenImpliesSeenByHigherPromiseQuorumHelper(c, v, chosenVB, inMsg, stepI, promMsg, promBal, i, stepJ, propMsg, accMsg, j);
}

lemma ChosenImpliesSeenByHigherPromiseQuorumHelper(c: Constants, v: Variables, chosenVB: ValBal, inMsg: Message, stepI: AcceptorHost.Step, promMsg: Message, promBal: LeaderId, i: nat, stepJ: AcceptorHost.Step, propMsg: Message, accMsg: Message, j: nat)
  requires RegularInvs(c, v)
