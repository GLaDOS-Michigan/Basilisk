...
module Obligations {
  ...
  ghost predicate SafetyAC3(c: Constants, v: Variables)
    requires v.WF(c)
  {
    forall pidx | c.ValidParticipantId(pidx) && ParticipantDecidedCommit(c, v.Last(), pidx)
    :: AllPreferYes(c)
  }
  ...
  ghost predicate SafetyAC4(c: Constants, v: Variables)
    requires v.WF(c)
  {
    forall pidx | c.ValidParticipantId(pidx) && PartipantHasDecided(c, v.Last(), pidx) && AllPreferYes(c)
    :: ParticipantDecidedCommit(c, v.Last(), pidx)
  }
